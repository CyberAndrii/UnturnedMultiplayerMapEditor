...
internal sealed class TellUpdateHeightmapPacket : INetSerializable, IDisposable
{
...
internal sealed class TellUpdateHeightmapRPC : IHostedService
{
    ...
    {
        _netPacketProcessor.SubscribeNetSerializable<TellUpdateHeightmapPacket, NetPeer>(Handle);
        return Task.CompletedTask;
    }

    public Task StopAsync(CancellationToken cancellationToken)
    {
        _netPacketProcessor.RemoveSubscription<TellUpdateHeightmapPacket>();
        return Task.CompletedTask;
    }

    private void Handle(TellUpdateHeightmapPacket packet, NetPeer peer)
    {
        try
        {
            ...
            _netHeightmapManager.ApplyDifferenceLocal(tile, packet.DifferenceHeightmap!);
        }
        finally
        {
            packet.Dispose();
